...
class FileHandler {
    ...
    public function __construct($typeFolder, $maxFilesInFolder = 500) {
        ...
        $this->yearFolder = $this->checkYearFolder($this->rootFolder);
    ...
    /**
     * Check if in root folder isset a folder with the current year date.
     * 
     * @param type $path
     * @return type
     */
    protected function checkYearFolder($path) {
        $yearFolders = array();
        ...
        foreach ($this->getIterator($path) as $info) {
            ...
            array_push($yearFolders, $info->getBasename());
        }

        if (!in_array(date('Y', time()), $yearFolders)) {
            mkdir($path . '/' . date('Y', time()), 0777);
        }
    ...
    protected function iterateImages() {
        ...
        if ($currentFilesNumber < $this->maxFilesInFolder) {
            $fileName = $this->getFileName($this->file->getClientOriginalName(), $currentFilesNumber);

            $this->file->move($this->filePath, $fileName);

            return UPLOAD_PATH . $this->typeFolder . '/'
                    . $this->yearFolder . '/'
                    . $this->lastFolder . '/' . $fileName;
        } else {
            ...
            $fileName = $this->getFileName($this->file->getClientOriginalName(), 0);

            $this->file->move($newPath, $fileName);
